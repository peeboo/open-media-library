...
IF EXIST ..\..\PostInstallerWizard\SQLInstallers\SQLEXPR_x64_ENU.exe GOTO BUILDSERVER
GOTO EXIT

:BUILDSERVER
REM Build the MSI for the server package
echo Building Server installer
"%WIX_BUILD_LOCATION%\candle.exe" -ext WiXNetFxExtension -ext WiXUtilExtension "..\Wix Fragments\OMLCustomActions.wxs" setupserver.wxs -ext WiXFirewallExtension setup.wxs -dBuildType=%BUILD_TYPE% -out %APP_INTERMEDIATE_PATH%\
